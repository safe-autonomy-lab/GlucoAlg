...
class ShieldedOnPolicyBuffer(BaseBuffer):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self.data['shield_violation'] = torch.zeros((size,), dtype=torch.float32, device=device)
        self.data['shield_adv_c'] = torch.zeros((size,), dtype=torch.float32, device=device)
        self.data['value_shield_violation'] = torch.zeros((size,), dtype=torch.float32, device=device)
        self.data['target_shield_violation'] = torch.zeros((size,), dtype=torch.float32, device=device)
    ...
    def finish_path(
        ...
        last_value_shield_violation: torch.Tensor | None = None,
    ) -> None:
        ...
        if last_value_shield_violation is None:
            last_value_shield_violation = torch.zeros(1, device=self._device)

        path_slice = slice(self.path_start_idx, self.ptr)
        last_value_r = last_value_r.to(self._device)
        last_value_c = last_value_c.to(self._device)
        last_value_shield_violation = last_value_shield_violation.to(self._device)
        ...
        # Shield extra
        shield_violation = torch.cat([self.data['shield_violation'][path_slice], last_value_shield_violation])
        value_shield_violation = torch.cat([self.data['value_shield_violation'][path_slice], last_value_shield_violation])

        shield_adv_c, target_shield_violation = self._calculate_adv_and_value_targets(
            value_shield_violation,
            shield_violation,
            lam=self._lam_c,
        )
        self.data['shield_adv_c'][path_slice] = shield_adv_c
        self.data['target_shield_violation'][path_slice] = target_shield_violation
    ...
    def get(self) -> dict[str, torch.Tensor]:
        ...
        data = {
            ...
            'shield_violation': self.data['shield_violation'],
            'shield_adv_c': self.data['shield_adv_c'],
            'target_shield_violation': self.data['target_shield_violation'],
        }
        ...
        shield_violation_mean, *_ = distributed.dist_statistics_scalar(data['shield_violation'])
        if self._standardized_adv_r:
            data['adv_r'] = (data['adv_r'] - adv_mean) / (adv_std + 1e-8)
        if self._standardized_adv_c:
            data['adv_c'] = data['adv_c'] - cadv_mean
        
        data['shield_adv_c'] = (data['shield_adv_c'] - shield_violation_mean)
