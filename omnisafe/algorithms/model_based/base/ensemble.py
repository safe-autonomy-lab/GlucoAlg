...
class EnsembleDynamicsModel:
    ...
    def save_model(self, path: str) -> None:
        """Save the ensemble model state and scaler state."""
        torch.save({
            'ensemble_model_state_dict': self._ensemble_model.state_dict(),
            'scaler_state': self._ensemble_model.scaler.save_state(),
            'elite_indices': self.elite_model_idxes,
            'val_losses': self._current_val_losses,
            'snapshots': self._snapshots
        }, path)
        print(f"World model saved to {path}")

    def load_model(self, path: str) -> None:
        """Load the ensemble model state and scaler state."""
        checkpoint = torch.load(path, map_location=self._device)
        self._ensemble_model.load_state_dict(checkpoint['ensemble_model_state_dict'])
        self._ensemble_model.scaler.load_state(checkpoint['scaler_state'])
        self.elite_model_idxes = checkpoint.get('elite_indices', list(range(self._config.elite_size)))
        self._current_val_losses = checkpoint.get('val_losses', np.full(self._config.num_ensemble, float('inf')))
        self._snapshots = checkpoint.get('snapshots', {i: (0, float('inf')) for i in range(self._config.num_ensemble)})
