...
class PPO(PolicyGradient):
    ...
    def _loss_pi(
        ...
        # Handle both continuous (Gaussian) and discrete (Categorical) actors
        if hasattr(self._actor_critic.actor, 'std'):
            # Continuous/Gaussian actor - log std for exploration info
            ...
            # Discrete/Categorical actor - no std, entropy is the main exploration metric
            self._logger.store(
                {
                    'Train/Entropy': entropy,
                    'Train/PolicyRatio': ratio,
