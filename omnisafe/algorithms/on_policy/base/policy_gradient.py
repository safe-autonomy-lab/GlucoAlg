...
class PolicyGradient(BaseAlgo):
    ...
    def _init_model(self) -> None:
        ...
        if hasattr(self._cfgs, 'shield_cfgs'):
            self.shield_type = self._cfgs.shield_cfgs['penalty_type']
        else:
        ...
        if hasattr(self._cfgs, 'shield_cfgs'):
            if self._cfgs.shield_cfgs['use_fe_representation']:
                ...
                dynamics_predictor = load_model(f'saved_files/dynamics_predictor/{env_info}-v0', 'dynamics_predictor')
                mo_predictor = None
    ...
    def _init(self) -> None:
        ...
        if isinstance(self._env.observation_space, gym.spaces.Dict):
            self._buf: VectorOnPolicyDictBuffer = VectorOnPolicyDictBuffer(
                obs_space=self._env.observation_space,
                act_space=self._env.action_space,
    ...
    def _init_log(self) -> None:
        ...
        if self._cfgs.model_cfgs.actor_type in ['gaussian_learning', 'categorical']:
            # Register PolicyStd for both continuous and discrete actors
            # For categorical actors, this will be 0.0 (dummy value)
    ...
    def _update(self) -> None:
        ...
        obs_before_update = obs
        original_obs_before_update = original_obs
        old_distribution = self._actor_critic.actor(obs, original_obs=original_obs)
        
        dataloader = DataLoader(
            dataset=TensorDataset(obs, act, logp, target_value_r, target_value_c, adv_r, adv_c, original_obs),
