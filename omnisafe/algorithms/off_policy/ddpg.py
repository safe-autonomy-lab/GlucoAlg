...
class DDPG(BaseAlgo):
    ...
    def _update_actor(  # pylint: disable=too-many-arguments
        ...
        if isinstance(self._env.action_space, (spaces.Discrete, spaces.MultiDiscrete)):
            ...
            # Original continuous action update
    ...
    def _update_actor_discrete(self, obs: torch.Tensor) -> None:
        """Update actor for discrete action spaces using Q-learning style updates.
        ...
        if hasattr(self._actor_critic.reward_critic, 'get_all_q_values'):
            all_q_values = self._actor_critic.reward_critic.get_all_q_values(obs)[0]  # First critic
        else:
            # Fallback: evaluate all possible actions manually
            all_q_values = self._get_all_q_values_manual(obs)
        
        # Create target action probabilities from Q-values using softmax
        temperature = getattr(self._cfgs.algo_cfgs, 'policy_temperature', 1.0)
        target_probs = F.softmax(all_q_values / temperature, dim=-1)
    ...
    def _get_all_q_values_manual(self, obs: torch.Tensor) -> torch.Tensor:
        """Manually compute Q-values for all possible actions.
        ...
        if isinstance(self._env.action_space, spaces.Discrete):
            ...
            for action_idx in range(n_actions):
                ...
                q_value = self._actor_critic.reward_critic(obs, action_tensor)[0]
