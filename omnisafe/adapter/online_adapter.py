...
class OnlineAdapter:
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        if hasattr(self._cfgs, 'env_cfgs') and self._cfgs.env_cfgs is not None:
            env_kwargs = self._cfgs.env_cfgs.todict()
            
        self._env: CMDP = make(env_id, num_envs=num_envs, device=self._device, **env_kwargs)
        ...
        if self._env.need_evaluation:
            self._eval_env = make(env_id, num_envs=1, device=self._device, **env_kwargs)
    ...
    def _wrapper(
        ...
        if obs_normalize:
            if isinstance(self._env.observation_space, gym.spaces.Dict):
                self._env = DictObsNormalize(self._env, device=self._device)
            else:
    ...
    def _wrapper_eval(
        ...
        if obs_normalize:
            if isinstance(self._eval_env.observation_space, gym.spaces.Dict):
                self._eval_env = DictObsNormalize(self._eval_env, device=self._device)
            else:
        ...
        if isinstance(self._eval_env.observation_space, gym.spaces.Dict):
            self._eval_env = DictUnsqueeze(self._eval_env, device=self._device)
        else:
