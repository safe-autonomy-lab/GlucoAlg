...
class OnPolicyAdapter(OnlineAdapter):
    ...
    def rollout(  # pylint: disable=too-many-locals
        ...
        for step in track(
            ...
            act, value_r, value_c, logp = agent.step(obs, original_obs=info['original_obs'])
                    ...
                    if not done:
                        ...
                        if epoch_end:
                            if isinstance(obs, dict):
                                ...
                                _, last_value_r, last_value_c, _ = agent.step(obs_dict, original_obs=info['original_obs'][idx])
                                last_value_r = last_value_r.squeeze(0)
                                last_value_c = last_value_c.squeeze(0)
                            else:
                                _, last_value_r, last_value_c, _ = agent.step(obs[idx], original_obs=info['original_obs'][idx])
                        ...
                        if time_out:
                            if isinstance(obs, dict):
                                ...
                                _, last_value_r, last_value_c, _ = agent.step(
                                    ...
                                    original_obs=info['original_obs'][idx],
                                )
