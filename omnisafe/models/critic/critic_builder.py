...
from omnisafe.models.critic.discrete_q_critic import DiscreteQCritic
from omnisafe.models.critic.q_critic_with_feature_extractor import QCriticWithFeatureExtractor
from omnisafe.models.critic.v_critic import VCritic
from omnisafe.models.critic.v_critic_with_feature_extractor import VCriticWithFeatureExtractor
...
class CriticBuilder:
    ...
    def build_critic(
        ...
        Currently, we support three types of critics: ``q``, ``discrete_q``, and ``v``.
        The ``discrete_q`` critic is automatically selected when the action space is discrete.

        Args:
            critic_type (str): Critic type.

        Returns:
            An instance of V-Critic, Q-Critic, or DiscreteQ-Critic

        Raises:
            NotImplementedError: If the critic type is not supported.
        """
        if critic_type == 'q':
            # Auto-select discrete Q-critic for discrete action spaces
            if isinstance(self._act_space, (spaces.Discrete, spaces.MultiDiscrete)):
                logger.info(f"Auto-selecting DiscreteQCritic for discrete action space: {type(self._act_space)}")
                return DiscreteQCritic(
                    ...
                    use_one_hot=True,  # Use one-hot encoding by default
                )
            else:
                return QCritic(
                    obs_space=self._obs_space,
        ...
        raise NotImplementedError(
            ...
            'Available critic types are: "q", "discrete_q", "v".',
        )
