...
class ActionScale(Wrapper):
    ...
    For continuous action spaces (Box), scales the actions to the specified range.
    For discrete action spaces (Discrete/MultiDiscrete), this wrapper passes actions through unchanged.

    Examples:
        >>> env = ActionScale(env, low=-1, high=1)  # For continuous actions
        >>> env.action_space
        Box(-1.0, 1.0, (1,), float32)
        
        >>> env = ActionScale(env, low=0, high=1)  # For discrete actions (no scaling applied)
        >>> env.action_space
        Discrete(5)  # Original discrete space preserved

    Args:
        env (CMDP): The environment to wrap.
        device (torch.device): The device to use.
        low (int or float): The lower bound of the action space (only applies to Box spaces).
        high (int or float): The upper bound of the action space (only applies to Box spaces).
    """
    ...
    def __init__(
        ...
        self._is_discrete = isinstance(self.action_space, (spaces.Discrete, spaces.MultiDiscrete))
        
        if self._is_discrete:
            # For discrete actions, no scaling is needed - pass through original action space
            self._action_space = self.action_space
        else:
            # For continuous actions, apply scaling as before
            assert isinstance(self.action_space, spaces.Box), 'Continuous action space must be Box'
    ...
    def step(
        ...
        .. note::
            For continuous actions: The action will be scaled to the original range for agent training.
            For discrete actions: The action is passed through unchanged.
        ...
        # For discrete actions, pass through unchanged
...
class Unsqueeze(Wrapper):
    ...
    Works with both continuous (Box) and discrete (Discrete/MultiDiscrete) action spaces.
    ...
    def step(
        ...
        .. note::
            ...
            For discrete actions, the action tensor is properly handled to maintain discrete values.
