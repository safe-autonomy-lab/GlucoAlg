...
class BaseDataset:
    ...
    def check_dataset(self):
        ...
        assert tuple(example_xs.shape) == (self.n_functions, self.n_examples, *self.input_size), f"Expected example_xs shape to be {(self.n_functions, self.n_examples, *self.input_size)}, got {example_xs.shape}"
        assert tuple(example_ys.shape) == (self.n_functions, self.n_examples, *self.output_size), f"Expected example_ys shape to be {(self.n_functions, self.n_examples, *self.output_size)}, got {example_ys.shape}"
        assert tuple(query_xs.shape) == (self.n_functions, self.n_queries, *self.input_size), f"Expected xs shape to be {(self.n_functions, self.n_queries, *self.input_size)}, got {tuple(query_xs.shape)}"
        assert tuple(query_ys.shape) == (self.n_functions, self.n_queries, *self.output_size), f"Expected ys shape to be {(self.n_functions, self.n_queries, *self.output_size)}, got {tuple(query_ys.shape)}"
